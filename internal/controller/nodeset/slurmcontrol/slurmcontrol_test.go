...
var _ = Describe("SlurmControlInterface", func() {
	...
	Context("DeleteNodeIfFailed()", func() {
		It("Should delete Slurm node", func() {
			By("Setup initial system state")
			nodeset = newNodeSet("foo", clusterName, 1)
			pod = nodesetutils.NewNodeSetPod(nodeset, 0, "")
			slurmNodename := nodesetutils.GetNodeName(pod)
			...
			By("Delete failed Slurm node")
			pods := []*corev1.Pod{pod}
			err := slurmcontrol.DeleteDefunctNodes(ctx, nodeset, pods)
			Expect(err).ToNot(HaveOccurred())

			By("Check Slurm Node exist")
			checkNode := &types.V0041Node{}
			key := object.ObjectKey(slurmNodename)
			err = sclient.Get(ctx, key, checkNode)
			Expect(err).To(HaveOccurred())
			isNotFound := err.Error() == http.StatusText(http.StatusNotFound)
			Expect(isNotFound).To(BeTrue())
			isEmpty := apiequality.Semantic.DeepEqual(checkNode, &types.V0041Node{})
			Expect(isEmpty).To(BeTrue())
		})

		It("Should ignore non-failed Slurm node", func() {
			By("Setup initial system state")
			nodeset = newNodeSet("foo", clusterName, 1)
			pod = nodesetutils.NewNodeSetPod(nodeset, 0, "")
			slurmNodename := nodesetutils.GetNodeName(pod)
